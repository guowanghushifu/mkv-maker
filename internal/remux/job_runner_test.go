...
func TestClearDirectoryContentsIfPresentPreservesRootDirectory(t *testing.T) {
	...
	childDir := filepath.Join(tempDir, "nested")
	childNestedFile := filepath.Join(childDir, "old.txt")

	if err := os.WriteFile(childFile, []byte("stale"), 0o644); err != nil {
		t.Fatalf("WriteFile failed: %v", err)
	}
	if err := os.MkdirAll(childDir, 0o755); err != nil {
		t.Fatalf("MkdirAll failed: %v", err)
	}
	if err := os.WriteFile(childNestedFile, []byte("nested"), 0o644); err != nil {
		t.Fatalf("WriteFile failed: %v", err)
	}

	if err := clearDirectoryContentsIfPresent(tempDir); err != nil {
		t.Fatalf("clearDirectoryContentsIfPresent returned error: %v", err)
	}
	...
	if !info.IsDir() {
		t.Fatalf("expected %q to remain a directory", tempDir)
	}
...
func TestJobRunnerExecuteClearsTempDirContentsButPreservesDirectory(t *testing.T) {
	outputRoot := t.TempDir()
	finalPath := filepath.Join(outputRoot, "Disc.mkv")
	intermediateDir := filepath.Join(outputRoot, "makemkv")
	staleFile := filepath.Join(intermediateDir, "stale.txt")
	staleDir := filepath.Join(intermediateDir, "old")
	staleNestedFile := filepath.Join(staleDir, "orphan.mkv")
	...
	if err := os.WriteFile(staleNestedFile, []byte("orphan"), 0o644); err != nil {
		t.Fatalf("WriteFile failed: %v", err)
	}
	...
	jobRunner.runMakeMKVMKV = func(_ context.Context, sourcePath string, titleID int, tempDir string, onOutput func(string)) error {
		if _, err := os.Stat(intermediateDir); err != nil {
			t.Fatalf("expected temp dir to exist before MakeMKV writes: %v", err)
		}
		...
		if _, err := os.Stat(staleDir); !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("expected stale directory to be removed, got %v", err)
		}
	...
	if _, err := os.Stat(intermediateDir); err != nil {
		t.Fatalf("expected temp dir to remain after Execute: %v", err)
	}
}
...
func TestDefaultRunMakeMKVCommandsSetConfigHomeWithoutFaketime(t *testing.T) {
	t.Setenv("FAKETIME", "")
	...
	if _, err := jobRunner.defaultRunMakeMKVInfo(context.Background(), "/bd_input/Disc/BDMV"); err != nil {
		t.Fatalf("defaultRunMakeMKVInfo returned error: %v", err)
	}
	if err := jobRunner.defaultRunMakeMKVMKV(context.Background(), "/bd_input/Disc/BDMV", 4, t.TempDir(), nil); err != nil {
		t.Fatalf("defaultRunMakeMKVMKV returned error: %v", err)
	...
	if strings.Count(logged, "HOME=/config\n") != 2 {
		t.Fatalf("expected HOME=/config twice, got %q", logged)
	}
	if strings.Count(logged, "LD_PRELOAD=<unset>\n") != 2 {
		t.Fatalf("expected LD_PRELOAD to stay unset twice, got %q", logged)
	}
	if strings.Count(logged, "FAKETIME=<unset>\n") != 2 {
		t.Fatalf("expected FAKETIME to stay unset twice, got %q", logged)
	}
	if strings.Count(logged, "FAKETIME_DONT_FAKE_MONOTONIC=<unset>\n") != 2 {
		t.Fatalf("expected FAKETIME_DONT_FAKE_MONOTONIC to stay unset twice, got %q", logged)
	}
}

func TestDefaultRunMakeMKVCommandsAddConfiguredFaketime(t *testing.T) {
	...
	if strings.Count(logged, "LD_PRELOAD=/usr/local/lib/libfaketime.so.1\n") != 2 {
		t.Fatalf("expected LD_PRELOAD twice, got %q", logged)
	}
	if strings.Count(logged, "FAKETIME=@2026-04-10 00:00:00\n") != 2 {
		t.Fatalf("expected FAKETIME twice, got %q", logged)
	}
	if strings.Count(logged, "FAKETIME_DONT_FAKE_MONOTONIC=1\n") != 2 {
		t.Fatalf("expected FAKETIME_DONT_FAKE_MONOTONIC twice, got %q", logged)
	}
}
